...
public class WebSecurityConfig {
	...
	protected UserDetailsService userDetailsService() {
		...
		UserDetails admin = User.builder().username("ADMIN")
										...
										.roles("ADMIN","USER")
										.build();
		UserDetails hr = User.builder().username("HR")
				.password(passwordEncoder().encode("hr"))
				.roles("ADMIN","USER")
				.build();
		UserDetails emp = User.builder().username("Employee")
	...
	// csrf ?? => a policy that aims to prevent cross site request forger protection web applications.
	@Bean
